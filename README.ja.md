...
- **タスクフロー** — 1 つのポートで独立したタスクの順次パイプラインを実行。各タスクは独自のモデル+プロンプトを持ち、前の出力が次へ渡されます。タスクごとに**固定**（設定したモデル）か**モデルプール**（リクエストの `model` フィールドで選択、許可リストで制限可）。UI で編集でき、専用の「タスクフロー」ページもあります。各タスクには展開可能な**「詳細（入出力フォーマット）」**パネルがあり、生成パラメータ（`temperature` / `top_p` / `max_tokens` / サンプリング）、入力・画像処理（画像精度・ソース・動画フレーム数・2段強制）、出力フォーマット（件数・1件の文字数上限・JSON 強制）を調整可能。空欄＝テンプレート既定、変更はホットスワップ。
...
- **ポート競合ガード** — 他プロセスが使用中のポートは「使用中」と表示され起動できません。
- **詳細デバッグログ** — ポートごとに切替でき、完全な（省略なし）リクエスト / レスポンス / エラーを記録。
- **既定の入出力フォーマット表示** — ポート編集画面のプロンプトメニューに、テンプレートのエンドポイント一覧・入出力 JSON 例・完全な既定プロンプトを読み取り専用で表示（「出力フォーマットはプロンプトで決まる」と明記）。ワンクリックで既定プロンプトを適用可能。
- **ステージ別の完全プロンプト編集** — プロンプトタブで各パイプラインステージごとに完全な既定プロンプトを表示・編集（ステージ単位で既定の表示/復元）。
- **モジュール式ルーティング** — 「ルート」タブ（タスクフロー風）で各パスをサブメニューから設定：組み込み機能の選択、公開パス、用途、有効/無効、さらに各パス専用の完全なタスクフロー（パスごとにモデル/プロンプト/パラメータを上書き）も作成可能。空欄ならネイティブ既定パス。
- **グローバル直列処理** — プロセス全体のガード：同時に処理するポートリクエストは最大1件、残りはキュー（モデル推論の同時実行なし）。GET/ヘルスは対象外。既定の上限は1、`HUB_GLOBAL_MAX_CONCURRENCY` で変更可（再起動で反映）。
- **ローカルモデルの自動アンロード** — 既定オン：各リクエスト後にローカルモデルをアンロードして VRAM を解放（Ollama は keep_alive=0、LM Studio は JIT ttl）。設定にトグル + ttl 秒。クラウド/OneAPI は対象外。
- **テンプレート設定** — ポート編集の新タブ：テンプレートが `params_schema` で全挙動設定（数値/トグル/テキスト/長文/JSON）を宣言し、動的フォームとして表示。空欄=既定値、保存で即時反映。カスタムパラメータの新規作成にも対応（任意のキー/値、テキスト/数値/JSON）、どのプロンプトからも `[[param:キー]]` で参照——コード変更なしで拡張可能。タスクフロー/ステージ別プロンプト/ルートと合わせ、UI だけでテンプレートをゼロから再現・カスタマイズ可能。
- **テンプレート分類** — テンプレートを 汎用/エージェント/評価 の3分類に：ポート作成のテンプレート選択がグループ表示になり、agent 系ポートは一覧に「Agent」バッジを表示。将来のエージェント・テンプレート群への布石。

**PromptLab（プロンプト逆生成）**
- 「入力→出力」サンプル（画像対応）から**システムプロンプトを逆推論**。制約の選択、再現テスト、ライブラリ保存、ポートへのワンクリック適用。

**インポートウィザード / Import Wizard**
- 既存 AI サービスのソースを貼り付けると、LLM がプロンプト/モデル/フローを抽出し、インポート可能なポート設定（タスクフロー付き）を生成 — 編集してワンクリック導入。空欄は自動補完（名前から slug、空きポート、既定のアプリ種別/別名）。
