...
- **テンプレート設定** — ポート編集の新タブ：テンプレートが `params_schema` で全挙動設定（数値/トグル/テキスト/長文/JSON）を宣言し、動的フォームとして表示。空欄=既定値、保存で即時反映。タスクフロー/ステージ別プロンプト/ルートと合わせ、UI だけでテンプレートをゼロから再現・カスタマイズ可能。
