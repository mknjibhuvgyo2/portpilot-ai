...
- **Template params** — a new "Template Params" tab in the port editor: templates declare all their behavior knobs via `params_schema` (number/bool/text/textarea/JSON), rendered as a dynamic form; empty = template default, saves hot-swap. Together with task flows / per-stage prompts / modular routes, a template can be rebuilt from scratch and customized in the UI.
