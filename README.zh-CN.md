...
- **任务流**：一个端口可串联一条有序任务流水线，每个任务独立配模型+提示词，上一步输出喂下一步。每个任务两种模式：**固定**（用配置的模型）或**模型池**（由调用方请求体的 `model` 字段选模型，可加允许名单约束）。UI 内可编辑，并有独立的「任务流」页面。每个任务还有可展开的**「高级（输入/输出格式）」**面板——生成参数（`temperature` / `top_p` / `max_tokens` / 采样方式）、输入图片处理（图片精度、来源、视频抽帧数、强制两段）、输出格式（条数、单条字数上限、强制 JSON）；留空＝用模板默认，改动热生效。
...
- **默认输入/输出格式查看**：端口编辑器→提示词菜单内只读展示该模板的接口列表、输入/输出 JSON 示例与完整默认提示词，并提示「输出格式由提示词决定」，可一键用默认提示词覆盖。
- **逐阶段完整提示词编辑**：提示词标签页按任务阶段逐个显示完整默认提示词并可编辑，每段可查看/恢复默认。
- **模块化接口路由**：端口编辑「接口路由」标签像任务流一样，每个路径点开**子菜单**配置——选内置功能、自定义对外路径、写用途、启用/停用，并可**为每个路径单独编一条完整任务流**（按路径覆盖端口默认的模型/提示词/参数）；留空即用模板原生默认路径。
- **全局串行处理**：进程级约束，整机同一时刻最多处理一个端口请求，其余自动排队、禁止并发；GET/健康检查不排队。默认上限 1，可用环境变量 `HUB_GLOBAL_MAX_CONCURRENCY` 调整后重启。
- **本地模型跑完自动卸载**：默认开，每个请求结束后自动卸载本地模型释放显存——Ollama 用 keep_alive=0 立即卸载，LM Studio 用 JIT ttl；设置页有开关与 ttl 秒数；云端/OneAPI 不受影响。
- **模板参数**：端口编辑新「模板参数」标签——模板通过 `params_schema` 声明自己的全部行为设置（数字/开关/文本/长文本/JSON），动态渲染成表单，留空用默认、保存即热生效；并支持**新建自定义参数**（任意键值，值可为文本/数字/JSON），在任何提示词里用 `[[param:键名]]` 引用即生效——不改代码即可扩展；配合任务流/逐阶段提示词/接口路由，可用 UI 从 0 复刻并自定义模板。
- **模板分类**：模板分 通用/智能体/评估 三类——新建端口的模板下拉按类分组，agent 类端口在列表中带「Agent」徽标，为未来的智能体模板家族铺路。
