...
// read-only list of the callable paths a port exposes (configured routes, then
// template defaults, then the generic OpenAI-compatible fallback).
...
// open the port editor at the Routes tab, where each path has a submenu to configure
// its function (handler + its own complete task flow).
function configureRoutes(p: any) { router.push({ path: '/ports', query: { edit: p.id, tab: 'routes' } }) }
  ...
  <div class="space-y-5">
    ...
    <div v-for="p in ports" :key="p.id" class="card space-y-3">
      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        <button v-if="auth.isAdmin && templateRoutesOf(p)" class="btn-ghost ml-auto" @click="configureRoutes(p)">
          <WaIcon name="sliders" :size="14" />{{ t('paths.configureRoutes') }}
        </button>
      </div>
      ...
      <!-- read-only endpoint overview: path + purpose + direct/gateway URLs -->
      <div class="overflow-x-auto rounded-lg border border-steel-200/70 dark:border-steel-800">
        <table class="w-full text-xs">
          ...
          <tbody>
            <tr v-for="ep in pathsOf(p)" :key="ep.path" class="border-t border-steel-100 align-top first:border-0 dark:border-steel-800/60">
              <td class="px-3 py-2">
              ...
              <td class="px-3 py-2 text-steel-500 dark:text-steel-400">{{ ep.desc || '—' }}</td>
              <td class="px-3 py-2">
